...
export async function GET(request: NextRequest) {
  ...
  try {
    // Use the airport search endpoint for hotel destinations as well
    ...
    // Transform data for hotel destinations
    const transformedData = {
      ...data,
      data:
        data.data?.map((location: any) => ({
          entityId: location.relevantHotelParams?.entityId || location.entityId,
          title: location.presentation?.title || location.navigation?.localizedName,
          subtitle: location.presentation?.subtitle,
        })) || [],
    }
